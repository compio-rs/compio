...
pub trait AsyncWriteAncillaryZerocopy {
    ...
    /// to be reused.
    fn write_with_ancillery_zerocopy<T: IoBuf, C: IoBuf>(
        &mut self,
        buf: T,
        control: C,
    ) -> impl Future<Output = BufResult<usize, Self::BufferReadyFuture<T, C>>>;

    /// Like `write_with_ancillery_zerocopy`, except that it writes from a
    /// buffer implements [`IoVectoredBuf`] into the source.
    fn write_vectored_with_ancillery_zerocopy<T: IoVectoredBuf, C: IoBuf>(
        &mut self,
        buf: T,
        control: C,
...
impl<A: AsyncWriteAncillaryZerocopy + ?Sized> AsyncWriteAncillaryZerocopy for &mut A {
    ...
    fn write_with_ancillery_zerocopy<T: IoBuf, C: IoBuf>(
        ...
        (**self).write_with_ancillery_zerocopy(buf, control)
    }
    ...
    fn write_vectored_with_ancillery_zerocopy<T: IoVectoredBuf, C: IoBuf>(
        ...
        (**self).write_vectored_with_ancillery_zerocopy(buf, control)
    }
}
