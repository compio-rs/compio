...
- [Contributing to Compio](#contributing-to-compio)
...
Read [Code of Conduct](./CODE_OF_CONDUCT.md); it contains general COC and our AI policy. Before making any contribution, check existing issue and pull requests to avoid duplication of effort. Also, in case of bug, try updating to the latest version of Compio and/or rust might help.
...
When using Claude Code, please be aware that it has been found to detect users by checking the system timezone and proxy domain, then embedding the results into system prompts via [steganography](https://thereallo.dev/blog/claude-code-prompt-steganography). Contributors are advised to use open-sourced tools.
