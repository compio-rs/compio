...
use crate::{
    ...
    hpack::DecodedHeader,
    state::{
        ConnShared, IncomingStream, SharedState, has_no_pseudo_headers, headers_to_header_map,
...
fn encode_data_frames(
    ...
    // It returns Ok(false) if flow control blocked, but our caller pre-checks
    // capacity.
    let sent = s.encode_data(stream_id, &data, end_stream)?;
    debug_assert!(
        sent || data.is_empty(),
        "caller should have checked flow control"
    );
    Ok(())
}
            ...
            if has_no_pseudo_headers(&decoded) && end_stream {
                ...
                for dh in &decoded {
                    if &dh.name[..] == b":status" {
                        ...
                        {
                            status_code =
                                http::StatusCode::from_u16(code).unwrap_or(http::StatusCode::OK);
                        }
...
fn validate_pseudo_headers(decoded: &[DecodedHeader], is_request: bool) -> Result<(), H2Error> {
    ...
    if is_request {
        ...
        if !is_connect {
            ...
            if decoded
                .iter()
                .any(|dh| &dh.name[..] == b":path" && dh.value.is_empty())
            {
        ...
        if seen_pseudos.contains(&b":status"[..]) {
            return Err(H2Error::Protocol(":status in request headers".into()));
        }
...
fn validate_regular_headers(decoded: &[DecodedHeader], stream_id: StreamId) -> Result<(), H2Error> {
    for dh in decoded {
        if dh.name.starts_with(b":") {
            continue;
        }
