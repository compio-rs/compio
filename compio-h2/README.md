...
- HPACK header compression with compile-time Huffman tables and O(1) hash-based dynamic table lookup
- Per-stream and connection-level flow control with send-side backpressure
- `ClientBuilder` / `ServerBuilder` with fluent configuration for all HTTP/2 settings
