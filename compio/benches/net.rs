...
criterion_group!(net, tcp, udp, multi_tcp, multi_udp);
criterion_main!(net);
...
fn multi_tcp(c: &mut Criterion) {
    const TASK_COUNT: u32 = 100;
    const PACKET_LEN: usize = 1048576;
    static PACKET: &[u8] = &[1u8; PACKET_LEN];

    let mut group = c.benchmark_group("multi_tcp");
    ...
    group.bench_function("tokio", |b| {
        ...
        b.to_async(&runtime).iter(|| async {
            ...
            let task = |listener: Arc<tokio::net::TcpListener>| async move {
                let addr = listener.local_addr().unwrap();
                let tx = tokio::net::TcpStream::connect(addr);
                let rx = listener.accept();
                let (mut tx, (mut rx, _)) = tokio::try_join!(tx, rx).unwrap();
            ...
            let handles = (0..TASK_COUNT)
                .map(|_| tokio::spawn(task(listener.clone())))
                .collect::<Vec<_>>();
    ...
    group.bench_function("compio", |b| {
        ...
        b.to_async(&runtime).iter(|| async {
            ...
            let task = |listener: Arc<compio::net::TcpListener>| async move {
                ...
                tx.write_all(PACKET).await.0.unwrap();
                let buffer = Vec::with_capacity(PACKET_LEN);
                _ = rx.read_exact(buffer).await.unwrap();
            };

            let handles = (0..TASK_COUNT)
                .map(|_| compio::runtime::spawn(task(listener.clone())))
                .collect::<Vec<_>>();
        ...
        b.iter(|| {
            runtime.block_on(async {
                ...
                let task = |listener: Arc<monoio::net::TcpListener>| async move {
                    ...
                    tx.write_all(PACKET).await.0.unwrap();
                    let buffer = Vec::with_capacity(PACKET_LEN);
                    _ = rx.read_exact(buffer).await;
                };

                let handles = (0..TASK_COUNT)
                    .map(|_| monoio::spawn(task(listener.clone())))
                    .collect::<Vec<_>>();
...
fn multi_udp(c: &mut Criterion) {
    const TASK_COUNT: u32 = 100;
    const PACKET_LEN: usize = 1024;
    static PACKET: &[u8] = &[1u8; PACKET_LEN];

    let mut group = c.benchmark_group("multi_udp");
    ...
    group.bench_function("compio", |b| {
        ...
        b.to_async(&runtime).iter(|| async {
            ...
            let handles = (0..TASK_COUNT)
                .map(|_| compio::runtime::spawn(task()))
                .collect::<Vec<_>>();
